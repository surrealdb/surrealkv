...
enum StoreOp {
    ...
    Range { start_idx: u8, end_idx: u8 },
    /// Reverse range query and verify results
    RangeReverse { start_idx: u8, end_idx: u8 },
...
fuzz_target!(|data: StoreFuzzInput| {
    ...
    let operations: Vec<StoreOp> = data
        ...
        .filter(|op| {
            match op {
                StoreOp::Set { value, .. } => value.len() <= 256,
                _ => true,
            }
        })
        .collect();
...
async fn run_fuzz_test(
    ...
    for op in operations {
        match op {
                    ...
                    Ok(result) => {
                        ...
                        match (result, expected_value) {
                            ...
                            (None, Some(Some(_))) => {
                                ...
                                panic!(
                                    "Get returned None for key {:?} that should exist",
                                    key
                                );
                            }
                            (None, Some(None)) => {
                            ...
                            (Some(got), None) => {
                                ...
                                panic!(
                                    "Get returned {:?} for never-set key {:?}",
                                    got, key
                                );
                            }
                        }
            ...
            StoreOp::Range { start_idx, end_idx } => {
                ...
                let iter = match txn.range(start_key.as_slice(), end_key.as_slice()) {
                    Ok(it) => it,
                ...
                assert_eq!(
                    ...
                    start_key, end_key, results.len(), expected_results.len()
                );
                ...
                for i in 1..results.len() {
                    assert!(
                        ...
                        "Range results not sorted at index {}", i
                    );
                }
                    ...
                    results.iter().zip(expected_results.iter()).enumerate()
                {
                    assert_eq!(
                        got_key, *exp_key,
                        "Range key mismatch at index {}", i
                    );
                    assert_eq!(
                        got_value, *exp_value,
                        "Range value mismatch at index {} for key {:?}", i, got_key
                    );
                }
            }

            StoreOp::RangeReverse { start_idx, end_idx } => {
                let start_idx = (*start_idx as usize) % keys.len();
                let end_idx = (*end_idx as usize) % keys.len();
                ...
                for i in 1..results.len() {
                    assert!(
                        results[i - 1].0 > results[i].0,
                        "RangeReverse results not reverse sorted at index {}", i
                    );
                }
                    ...
                    results.iter().zip(expected_results.iter()).enumerate()
                {
                    assert_eq!(
                        got_key, *exp_key,
                        "RangeReverse key mismatch at index {}", i
                    );
                    assert_eq!(
                        got_value, *exp_value,
                        "RangeReverse value mismatch at index {} for key {:?}", i, got_key
                    );
                }
            }
            ...
            Ok(result) => {
                match (result, expected_value) {
                    (Some(got), Some(exp)) => {
                        assert_eq!(
                            got.as_slice(), exp.as_slice(),
                            "Final verification: wrong value for key {:?}", key
                        );
                    }
                    (None, Some(_)) => {
                        panic!(
                            "Final verification: key {:?} should exist but returned None",
                            key
                        );
                    }
                    (None, None) => {
                    ...
                    (Some(got), None) => {
                        panic!(
                            "Final verification: deleted key {:?} returned {:?}",
                            key, got
                        );
                    }
                }
