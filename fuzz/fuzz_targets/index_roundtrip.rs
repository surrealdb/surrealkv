...
use surrealkv::{CompressionType, InternalKeyKind, Options};
	...
	for entry in &entries {
		...
		match result {
			Ok(Some((idx, block_with_key))) => {
				...
				assert!(idx < index.blocks.len(), "Lookup returned out-of-bounds index");
			...
			Ok(None) => {
				panic!("Lookup returned None for entry that should exist: {:?}", entry.user_key);
			}
			Err(e) => {
