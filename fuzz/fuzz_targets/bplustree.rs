	...
	for op in &data.operations {
		match op {
			...
			Operation::Get {
				...
				match tree.get(key) {
					Ok(result) => match (result, expected.get(key)) {
						(Some(got), Some(exp)) => {
							assert_eq!(
								got.as_ref(),
								exp.as_slice(),
								"Get returned wrong value for key len={}",
								key.len()
							);
						}
						(None, None) => {}
			...
			Operation::Delete {
				...
				match tree.delete(key) {
					Ok(result) => match (result.is_some(), expected.remove(key).is_some()) {
						...
						(false, true) => {
							panic!(
								"Delete failed to find key (len={}) that should exist",
								key.len()
							);
						}
					},
