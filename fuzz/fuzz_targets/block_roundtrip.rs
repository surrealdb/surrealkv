...
use surrealkv::{InternalKey, InternalKeyKind, InternalKeyRef, INTERNAL_KEY_SEQ_NUM_MAX};
...
fuzz_target!(|data: FuzzBlockInput| {
	...
	// ========================================
	{
		...
		while iter.is_valid() {
			let key = InternalKeyRef::from_encoded(iter.key_bytes());
			...
			assert_eq!(key.kind(), expected.kind, "Kind mismatch at index {}", count);
	...
	// ========================================
	{
		...
		while iter.is_valid() {
			...
			assert_eq!(
				key.user_key(),
				expected.user_key.as_slice(),
				"User key mismatch at reverse index {}",
				count
			);
			assert_eq!(
				key.seq_num(),
				expected.seq_num,
				"Sequence number mismatch at reverse index {}",
				count
			);
			...
			assert_eq!(
				iter.value_bytes(),
				expected.value.as_slice(),
				"Value mismatch at reverse index {}",
				count
			);
	...
	// ========================================
	{
		...
		for (idx, entry) in entries.iter().enumerate() {
			...
			iter.seek_internal(&seek_key).unwrap();

			assert!(iter.is_valid(), "Seek to existing key should succeed at index {}", idx);

			let found_key = InternalKeyRef::from_encoded(iter.key_bytes());
			assert_eq!(
				found_key.user_key(),
				entry.user_key.as_slice(),
				"Seek found wrong user_key at index {}",
				idx
			);
			assert_eq!(
				found_key.seq_num(),
				entry.seq_num,
				"Seek found wrong seq_num at index {}",
				idx
			);
			...
			assert_eq!(
				...
				"Seek found wrong value at index {}",
				idx
			);
		}
	}
	...
	// ========================================
	{
		...
		assert!(!iter.is_valid(), "Should be invalid after exhaustion");

		// Reset and verify can iterate again
		iter.reset();
		iter.seek_to_first().unwrap();
		assert!(iter.is_valid(), "Should be valid after reset and seek_to_first");
		...
		assert!(!iter.is_valid(), "Should be invalid after backward exhaustion");
	}
});
