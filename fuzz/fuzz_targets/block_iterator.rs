...
enum IterOp {
    SeekToFirst,
    SeekToLast,
    Seek { user_key: Vec<u8>, seq_num: u64 },
    Next,
    Prev,
    Reset,
}
...
fuzz_target!(|data: FuzzIteratorInput| {
    ...
    let entry_data: Vec<(Vec<u8>, Vec<u8>)> = entries
        .iter()
        .map(|e| (e.to_internal_key().encode(), e.value.clone()))
        .collect();
    ...
    let mut iter = match block.iter(data.keys_only) {
        Ok(it) => it,
        Err(_) => return,
    };
        ...
        match op {
            ...
            IterOp::Seek { user_key, seq_num } => {
                ...
                if iter.valid() {
                    ...
                    let expected_idx = entry_data
                        .iter()
                        .position(|(k, _)| internal_cmp.compare(k, &seek_key) != std::cmp::Ordering::Less);
                    ...
                    assert!(
                        entry_data.iter().all(|(k, _)| internal_cmp.compare(k, &seek_key) == std::cmp::Ordering::Less),
            ...
            IterOp::Next => {
                ...
                if iter.valid() {
                    ...
                    if let Ok(true) = iter.advance() {
                        assert!(
                            internal_cmp.compare(&prev_key, iter.key_bytes()) == std::cmp::Ordering::Less,
            ...
            IterOp::Prev => {
                ...
                if iter.valid() {
                    ...
                    if let Ok(true) = iter.prev() {
                        assert!(
                            internal_cmp.compare(iter.key_bytes(), &next_key) == std::cmp::Ordering::Less,
            ...
            IterOp::Reset => {
                iter.reset();
                direction = None;
                assert!(!iter.valid(), "After reset, iterator should be invalid");
            }
        }
    }

    // Verification passes
    verify_full_forward_iteration(&block, &internal_cmp, &entry_data, data.keys_only);
    verify_full_backward_iteration(&block, &internal_cmp, &entry_data, data.keys_only);
...
fn test_empty_block() {
    ...
    let mut builder = BlockWriter::new(4096, 16, Arc::clone(&internal_cmp));
    ...
    let _ = iter.seek_to_first();
    assert!(!iter.valid(), "Empty block iterator should be invalid after seek_to_first");
...
fn verify_full_forward_iteration(
    ...
    let mut iter = block.iter(keys_only).expect("Should create iterator");
...
fn verify_seek_boundaries(
    ...
    for (key, _) in entry_data {
        let _ = iter.seek(key);
        assert!(iter.valid(), "Seek to existing key should succeed");
        assert_eq!(
            iter.key_bytes(),
            key,
            "Seek to exact key should find that key"
        );
    }
}