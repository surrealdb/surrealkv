...
fuzz_target!(|data: FuzzLookupInput| {
	...
	let partitions = match &data.test_mode {
		...
		TestMode::DenseKeys {
			prefix,
		} => generate_dense_partitions(prefix, &data.partitions),
		TestMode::Sequential {
			start,
			count,
		} => generate_sequential_partitions(*start, *count),
	};

	// Filter empty keys
	let mut partitions: Vec<_> =
		partitions.into_iter().filter(|p| !p.separator_key.is_empty()).collect();
	...
	for (i, sep_key) in separator_keys.iter().enumerate() {
		...
		assert_eq!(&block.separator_key, sep_key, "Returned block should have matching separator");
	}
	...
	if let Some(first) = partitions.first() {
		...
		if let Some(query_key) = before_first {
			...
			if let Some((idx, _)) = result {
				...
				assert!(
					idx == 0
						|| internal_cmp.compare(&query_key, &separator_keys[0])
							== Ordering::Greater
				);
			}
		}
	...
	for i in 0..partitions.len().saturating_sub(1) {
		...
		if let Some(between) = between_key {
			...
			if let Some((idx, _)) = result {
				...
				assert!(
					idx > i
						|| internal_cmp.compare(&query_key, &separator_keys[i])
							!= Ordering::Greater,
...
fn test_seq_num_edge_cases(
	...
	for partition in partitions {
		...
		let query_max_seq =
			make_internal_key(&partition.separator_key, u64::MAX, InternalKeyKind::Set);
		...
		let query_min_seq = make_internal_key(&partition.separator_key, 0, InternalKeyKind::Set);
...
fn generate_boundary_partitions(base: &[FuzzPartition]) -> Vec<FuzzPartition> {
	...
	partitions.push(FuzzPartition {
		separator_key: vec![0x00],
		seq_num: 0,
	});
	partitions.push(FuzzPartition {
		separator_key: vec![0x7F],
		seq_num: 0,
	});
	partitions.push(FuzzPartition {
		separator_key: vec![0x80],
		seq_num: 0,
	});
	partitions.push(FuzzPartition {
		separator_key: vec![0xFF],
		seq_num: 0,
	});
