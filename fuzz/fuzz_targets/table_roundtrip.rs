...
fuzz_target!(|data: FuzzTableInput| {
    ...
    if let Some(ref filter) = table_arc.filter_reader {
        for (raw, _) in &entries {
            ...
            assert!(
                may_contain,
                "Filter must not have false negatives for inserted keys"
            );
        }
    }
});
...
fn verify_metadata(table: &Table, entries: &[(FuzzEntryRaw, FuzzEntry)]) {
    ...
    if let (Some(smallest), Some(largest)) = (&meta.smallest_point, &meta.largest_point) {
        ...
        assert_eq!(
            &smallest.user_key, first_key,
            "Smallest key mismatch in metadata"
        );
        assert_eq!(
            &largest.user_key, last_key,
            "Largest key mismatch in metadata"
        );
    }
}
        ...
        match result {
            ...
            Ok(None) => {
                ...
                if fuzz_entry.kind == InternalKeyKind::Set {
                    // Check if filter might have rejected it (shouldn't happen - no false negatives)
            ...
            Ok(None) => {
                ...
                let exists = entries.iter().any(|(raw, _)| {
                    raw.user_key == query.user_key
                        && raw.seq_num.min(INTERNAL_KEY_SEQ_NUM_MAX) <= query.seq_num.min(INTERNAL_KEY_SEQ_NUM_MAX)
                });
...
fn verify_forward_iteration(
    ...
    let mut iter = table.iter(keys_only, None);
    let mut count = 0;
    ...
    for item in &mut iter {
        let (key, value) = match item {
            Ok(kv) => kv,
            Err(e) => panic!("Forward iteration error: {:?}", e),
        };
        ...
        assert_eq!(
            ...
            "Forward iteration key mismatch at index {}", count
        );

        if !keys_only {
            assert_eq!(
                value, expected.0.value,
                "Forward iteration value mismatch at index {}", count
            );
        }

        prev_key = Some(key.encode());
        count += 1;
    }
...
fn verify_backward_iteration(
    ...
    while let Some(item) = iter.next_back() {
        let (key, value) = match item {
            Ok(kv) => kv,
            Err(e) => panic!("Backward iteration error: {:?}", e),
        };
        ...
        assert_eq!(
            key.user_key.as_slice(),
            expected.0.user_key.as_slice(),
            "Backward iteration key mismatch at reverse index {}", count
        );

        if !keys_only {
            assert_eq!(
                value, expected.0.value,
                "Backward iteration value mismatch at reverse index {}", count
            );
        }

        next_key = Some(key.encode());
        count += 1;
    }
