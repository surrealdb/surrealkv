...
use surrealkv::{InternalKey, InternalKeyKind, Options, INTERNAL_KEY_SEQ_NUM_MAX};
...
fuzz_target!(|data: FuzzGetCorrectnessInput| {
	...
	for query in &data.queries {
		...
		let query_key =
			InternalKey::new(query.user_key.clone(), query_seq, InternalKeyKind::Set, 0);
		...
		match result {
			Ok(Some((found_key, found_value))) => {
				...
				if let Some((expected_seq, expected_value)) =
					latest_by_user_key.get(&query.user_key)
				{
	...
	if let Some(last_key) = sorted_user_keys.last() {
		...
		let query_key =
			InternalKey::new(after_key.clone(), INTERNAL_KEY_SEQ_NUM_MAX, InternalKeyKind::Set, 0);
