...
fuzz_target!(|data: FuzzMultiVersionInput| {
	...
	for query in &data.queries {
		...
		let query_key = InternalKey::new(base.user_key.clone(), query_seq, InternalKeyKind::Set, 0);
		...
		let expected_version = entries
			.iter()
			.filter(|(k, _)| k.user_key == base.user_key && k.seq_num() <= query_seq)
		...
		match (result, expected_version) {
			(Ok(Some((found_key, found_value))), Some((expected_key, expected_value))) => {
				...
				if expected_key.kind() != InternalKeyKind::Delete {
					assert_eq!(&found_value, expected_value, "Version query returned wrong value");
				}
			}
			(Ok(None), None) => {
	...
	let mut iter = match table_arc.iter(None) {
		Ok(it) => it,
		Err(_) => return,
	};
	...
	while iter.valid() {
		...
		if let Some(ref prev_uk) = prev_user_key {
			match user_cmp.compare(prev_uk, &user_key) {
				Ordering::Less => {
	...
	assert_eq!(iter_count, entries.len(), "Iteration should return all versions");
});
