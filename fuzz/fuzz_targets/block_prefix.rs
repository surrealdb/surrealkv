    ...
    for op in &data.operations {
        match op {
            ...
            PrefixOp::VerifyPrefixCompression => {
                ...
                let mut verify_iter = block.iter(data.keys_only).expect("Should create iterator");
                let _ = verify_iter.seek_to_first();

                let mut idx = 0;
                while verify_iter.valid() {
...
fn generate_entries_from_pattern(common_prefix: &[u8], pattern: &SuffixPattern) -> Vec<FuzzEntry> {
    match pattern {
        SuffixPattern::Identical { suffix, count, seq_nums } => {
        ...
        SuffixPattern::IncrementingByte { start, count } => {
        ...
        SuffixPattern::Hierarchical { segments, depth } => {
        ...
        SuffixPattern::LongWithSmallDiff { base_suffix, diff_positions, count } => {
        ...
        SuffixPattern::Alternating { suffix_a, suffix_b, count } => {
            let count = (*count).min(100) as usize;
        ...
        SuffixPattern::GradualDivergence { base, divergence_points } => {
            let mut entries = Vec::new();
...
fn verify_full_iteration(
    ...
    while iter.valid() {
        ...
        assert_eq!(
            &curr_key,
            &entry_data[count].0,
            "Key mismatch at position {}", count
        );
        ...
        if !keys_only {
            assert_eq!(
                iter.value_bytes(),
                &entry_data[count].1,
                "Value mismatch at position {}", count
            );
        }
