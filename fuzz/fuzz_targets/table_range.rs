...
impl FuzzRange {
    ...
    fn expected_entries<'a>(
        ...
        entries
            .iter()
            .filter(|(raw, _)| self.key_in_range(&raw.user_key, user_cmp))
            .collect()
    }
}
...
fuzz_target!(|data: FuzzRangeIterInput| {
    ...
    for range in &data.ranges {
        ...
        if range.test_forward {
            let mut iter = table.iter(data.keys_only, Some(ik_range.clone()));
            let mut results: Vec<(InternalKey, Vec<u8>)> = Vec::new();

            for item in &mut iter {
                match item {
                    Ok(kv) => results.push(kv),
                    Err(_) => break,
                }
            }
            ...
            for (raw, _) in &expected {
                let found = results.iter().any(|(k, _)| k.user_key == raw.user_key);
                assert!(
                    found,
                    "Forward iteration missing expected key: {:?}",
                    raw.user_key
                );
            }
        }
        ...
        if range.test_backward {
            ...
            assert_eq!(
                results.len(),
                expected.len(),
                "Backward range iteration count mismatch"
            );
