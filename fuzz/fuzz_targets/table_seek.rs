...
fuzz_target!(|data: FuzzSeekInput| {
    ...
    entries.sort_by(|a, b| {
        internal_cmp.compare(&a.1.encode(), &b.1.encode())
    });

    entries.dedup_by(|a, b| {
        internal_cmp.compare(&a.1.encode(), &b.1.encode()) == Ordering::Equal
    });
        ...
        match op {
            ...
            SeekOp::Seek { user_key, seq_num } => {
                ...
                if iter.seek(&encoded).is_ok() && iter.valid() {
                    let found_encoded = iter.key().encode();
            ...
            SeekOp::SeekBetween(idx1, idx2) => {
                ...
                if let Some(between) = create_key_between(key1, key2) {
                    ...
                    if iter.seek(&seek_key.encode()).is_ok() && iter.valid() {
                        let found = iter.key();
                        let found_encoded = found.encode();

                        assert!(
                            internal_cmp.compare(&found_encoded, &seek_key.encode()) != Ordering::Less,
