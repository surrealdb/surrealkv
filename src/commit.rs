...
mod tests {
	...
	fn test_sync_commit_sequence_number_consistency() {
		...
		batch1.add_record(InternalKeyKind::Set, b"key1", Some(b"value1")).unwrap();

		let mut batch2 = Batch::new();
		batch2.add_record(InternalKeyKind::Set, b"key2", Some(b"value2")).unwrap();
		batch2.add_record(InternalKeyKind::Set, b"key3", Some(b"value3")).unwrap();
