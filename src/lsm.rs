...
impl Core {
	...
	pub(crate) fn new(opts: Arc<Options>) -> Result<Self> {
		...
		let task_manager = Arc::new(TaskManager::new(Arc::clone(&inner) as Arc<dyn CompactionOperations>));

		let commit_env = Arc::new(LsmCommitEnv::new(Arc::clone(&inner), Arc::clone(&task_manager))?);
		...
		if let Some(ref vlog) = inner.vlog {
			let vlog_gc_manager = VLogGCManager::new(Arc::clone(vlog), Arc::clone(&commit_pipeline));
