...
mod tests {
	...
	async fn test_compaction_with_updates_and_delete() {
		...
		for (i, key) in keys.iter().enumerate() {
			...
			assert_eq!(result, Some(expected.as_bytes().into_bytes()));
		}
	...
	async fn test_compaction_with_updates_and_delete_on_same_key() {
		...
		for (i, key) in keys.iter().enumerate() {
			let tx = tree.begin().unwrap();
			let result = tx.get(*key).unwrap();
			assert_eq!(result, Some(format!("value-{}-v4", i + 1).as_bytes().into_bytes()));
		}
	...
	async fn test_crash_before_flush_replays_wal() {
		...
		// Phase 2: Restart and verify WAL was replayed
		{
			...
			let txn = tree.begin().unwrap();
			assert_eq!(txn.get(b"crash_key").unwrap(), Some(b"crash_value".into_bytes()));
	...
	async fn test_clean_shutdown_no_empty_wal() {
		...
		// Verify restart works and data is accessible
		{
			let tree = Tree::new(opts.clone()).unwrap();
			let txn = tree.begin().unwrap();
			assert_eq!(txn.get(b"key_0").unwrap(), Some(b"value".into_bytes()));
			assert_eq!(txn.get(b"extra").unwrap(), Some(b"data".into_bytes()));
			tree.close().await.unwrap();
		}
	}
	...
	async fn test_multiple_flush_cycles_log_number_sequence() {
		...
		// Restart and verify all data accessible from SSTables
		{
			...
			assert_eq!(txn.get(b"batch1_key_0").unwrap(), Some(b"value1".into_bytes()));
			assert_eq!(txn.get(b"batch2_key_0").unwrap(), Some(b"value2".into_bytes()));
			assert_eq!(txn.get(b"batch3_key_0").unwrap(), Some(b"value3".into_bytes()));
	...
	async fn test_shutdown_with_empty_memtable() {
		...
		// Restart successfully
		{
			let tree = Tree::new(opts.clone()).unwrap();
			let txn = tree.begin().unwrap();
			assert_eq!(txn.get(b"key").unwrap(), Some(b"value".into_bytes()));
			tree.close().await.unwrap();
		}
	}
	...
	async fn test_full_crash_recovery_scenario() {
		...
		// Phase 4: Restart and verify
		{
			...
			let txn = tree.begin().unwrap();
			assert_eq!(txn.get(b"batch_a_0").unwrap(), Some(b"value_a".into_bytes()));
			assert_eq!(txn.get(b"batch_b_0").unwrap(), Some(b"value_b".into_bytes()));
			assert_eq!(
				txn.get(b"batch_c_0").unwrap(),
				Some(b"value_c".into_bytes()),
	...
	async fn test_flush_on_close_creates_sst() {
		...
		// Phase 2: Reopen and verify data is in SST (not WAL)
		{
			...
			let txn = tree.begin().unwrap();
			assert_eq!(txn.get(b"test_key").unwrap(), Some(b"test_value".into_bytes()));
	...
	async fn test_multiple_flush_cycles_with_sst_and_wal_verification() {
		...
		// Cycle 2: Reopen, verify recovery, write more, flush, close
		{
			...
			let txn = tree.begin().unwrap();
			assert_eq!(txn.get(b"cycle1_key_0").unwrap(), Some(b"value1".into_bytes()));
			drop(txn);
		...
		// Cycle 3: Reopen, write but DON'T flush, close (tests shutdown flush)
		{
			...
			let txn = tree.begin().unwrap();
			assert_eq!(txn.get(b"cycle1_key_0").unwrap(), Some(b"value1".into_bytes()));
			assert_eq!(txn.get(b"cycle2_key_0").unwrap(), Some(b"value2".into_bytes()));
			drop(txn);
		...
		// Final verification: All data accessible from SSTs
		{
			...
			assert_eq!(txn.get(b"cycle1_key_0").unwrap(), Some(b"value1".into_bytes()));
			assert_eq!(txn.get(b"cycle2_key_0").unwrap(), Some(b"value2".into_bytes()));
			assert_eq!(txn.get(b"cycle3_key_0").unwrap(), Some(b"value3".into_bytes()));
	...
	async fn test_flush_on_close_option_comparison() {
		...
		// Test with flush_on_close = false
		{
			...
			let txn = tree.begin().unwrap();
			assert_eq!(txn.get(b"test").unwrap(), Some(b"data".into_bytes()));
	...
	async fn test_cleanup_orphaned_sst_files() {
		...
		// Reopen database - should trigger cleanup
		{
			...
			assert_eq!(result, b"value1".into_bytes());
	...
	async fn test_wal_recovery_mode_tolerate_with_repair_succeeds_on_corruption() {
		...
		// Phase 3: Open with default TolerateCorruptedWithRepair mode - should SUCCEED
		{
			...
			let txn = tree.begin().unwrap();
			assert_eq!(txn.get(b"key1").unwrap(), Some(b"value1".into_bytes()));
			assert_eq!(txn.get(b"key2").unwrap(), Some(b"value2".into_bytes()));
	...
	async fn test_recovery_with_manually_created_wal_segments() {
		...
		let highest_segment_created;
		{
			...
			let next_seq = last_seq_after_phase1 + 1;
			{
				...
				let encoded_value =
					ValueLocation::with_inline_value(b"value2_from_wal".into_bytes()).encode();
				batch
					.add_record(
						InternalKeyKind::Set,
						b"key2".into_bytes(),
						Some(encoded_value.into()),
						0,
					)
					.unwrap();
			...
			let segment_for_key3 = segment_for_key2 + 1;
			{
				...
				let encoded_value =
					ValueLocation::with_inline_value(b"value3_from_wal".into_bytes()).encode();
				batch
					.add_record(
						InternalKeyKind::Set,
						b"key3".into_bytes(),
