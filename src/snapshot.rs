...
impl SnapshotIterator<'_> {
    ...
    fn new_from<R>(core: Arc<Core>, seq_num: u64, range: R) -> Result<Self>
    where
...
mod tests {
    ...
    async fn test_snapshot_with_complex_key_patterns() {
        ...
        let numeric_range: Vec<_> = snapshot
            .range(b"000", b"999", None)
            .unwrap()
            .collect::<Vec<_>>();
        assert_eq!(numeric_range.len(), 10);

        let alpha_range: Vec<_> = snapshot
            .range(b"a", b"z", None)
            .unwrap()
            .collect::<Vec<_>>();
        assert_eq!(alpha_range.len(), 15); // 10 numeric + 10 alpha + 5 mixed

        let mixed_range: Vec<_> = snapshot
            .range(b"mix", b"miy", None)
            .unwrap()
            .collect::<Vec<_>>();
    ...
    fn test_range_skips_non_overlapping_tables() {
        ...
        let range = (
            Bound::Included(b"z0".to_vec()),
            Bound::Included(b"zz".to_vec()),
        );
