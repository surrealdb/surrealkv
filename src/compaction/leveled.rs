...
mod tests {
	...
	use crate::{
		...
		vlog::ValueLocation,
		CompressionType, Options as LSMOptions, ReadOptions,
	};
	...
	fn create_ordered_entries(
		...
		for i in 0..count {
			...
			let internal_key =
				InternalKey::new(Bytes::from(key), seq_num + i as u64, InternalKeyKind::Set, 0);
			let encoded_value = create_inline_value(&value);
	...
	fn generate_entries(
		...
		for i in 0..keys_per_table {
			...
			let internal_key = InternalKey::new(Bytes::from(key), seq_num, InternalKeyKind::Set, 0);
	...
	async fn test_multi_level_merge_compaction() {
		...
		fn generate_entries(
			...
			for i in 0..keys_per_table {
				...
				let internal_key =
					InternalKey::new(Bytes::from(key), seq_num, InternalKeyKind::Set, 0);
		...
		for (level_idx, level) in levels.iter().enumerate().skip(1) {
			if level.tables.len() >= 2 {
				...
				for table in &level.tables {
					...
					for (key, _) in table.iter(ReadOptions::default()) {
						if min_key.is_none() {
	...
	async fn test_compaction_with_large_keys_and_values() {
		...
		for i in 0..5 {
			...
			let internal_key = InternalKey::new(Bytes::from(key), 1000, InternalKeyKind::Set, 0);
	...
	async fn test_l0_overlapping_keys_compaction() {
		...
		for i in 5..=15 {
			...
			let internal_key = InternalKey::new(Bytes::from(key), 100 + i, InternalKeyKind::Set, 0);
		...
		for i in 10..=20 {
			...
			let internal_key =
				InternalKey::new(Bytes::from(key), 150 + i - 10, InternalKeyKind::Set, 0);
		...
		for i in 8..=12 {
			...
			let internal_key =
				InternalKey::new(Bytes::from(key), 200 + i - 8, InternalKeyKind::Set, 0);
		...
		let tombstone =
			InternalKey::new(Bytes::from(tombstone_key), 210, InternalKeyKind::Delete, 0);
		...
		for level in levels {
			for table in &level.tables {
				for (key, encoded_value) in table.iter(ReadOptions::default()) {
					match key.kind() {
						InternalKeyKind::Set => {
	...
	async fn test_l0_tombstone_propagation_overlapping() {
		...
		for i in 0..20 {
			...
			entries1.push((
				InternalKey::new(Bytes::from(key), 100 + i, InternalKeyKind::Set, 0),
				encoded_value,
			));
		}
		...
		for i in 5..15 {
			...
			entries2.push((InternalKey::new(Bytes::from(key), 150 + i - 5, kind, 0), value));
		}
		...
		for i in [2, 8, 14, 17] {
			let key = format!("key-{i:03}").into_bytes();
			entries3.push((
				InternalKey::new(Bytes::from(key), 200 + i / 2, InternalKeyKind::Delete, 0),
				vec![],
			));
		}
	...
	async fn test_tombstone_propagation_through_levels() {
		...
		for table_idx in 0..4 {
			...
			for i in (table_idx * 3)..((table_idx + 1) * 3) {
				...
				l2_entries.push((InternalKey::new(Bytes::from(key), seq, kind, 0), value));
			}
	...
	fn test_tombstone_propagation_journey() {
		...
		let iterators: Vec<_> = vec![
			Box::new(tombstone_table.iter(ReadOptions::default()))
				as Box<dyn DoubleEndedIterator<Item = _>>,
			Box::new(value_table.iter(ReadOptions::default()))
				as Box<dyn DoubleEndedIterator<Item = _>>,
		];
	...
	fn test_table_properties_population() {
		...
		for i in 0..100 {
			...
			let internal_key = InternalKey::new(Bytes::from(key), seq, kind, 0);
	...
	async fn test_soft_delete_compaction_behavior() {
		...
		for table_idx in 0..2 {
			...
			for i in (table_idx * 6)..((table_idx + 1) * 6) {
				...
				l0_entries.push((InternalKey::new(Bytes::from(key), seq, kind, 0), value));
			}
		...
		for table in &levels[1].tables {
			for (key, value) in table.iter(ReadOptions::default()) {
				match key.kind() {
					InternalKeyKind::Set => {
