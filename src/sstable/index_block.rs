...
impl BlockHandleWithKey {
    ...
    pub fn new(user_key: Vec<u8>, handle: BlockHandle) -> BlockHandleWithKey {
        BlockHandleWithKey { user_key, handle }
    }
...
impl TopLevelIndexWriter {
    ...
    pub fn size_estimate(&self) -> usize {
        ...
        let finished_blocks_size: usize = self
            .index_blocks
            .iter()
            .map(|block| block.size_estimate())
            .sum();
...
impl TopLevelIndex {
    ...
    pub fn find_block_handle_by_key(&self, user_key: &[u8]) -> Option<&BlockHandleWithKey> {
        ...
        let index = self
            .blocks
            .partition_point(|block| block.user_key.as_slice() < user_key);
    ...
    pub fn load_block(&self, block_handle: &BlockHandleWithKey) -> Result<Arc<Block>> {
        if let Some(block) = self
            .opts
            .block_cache
            .get_index_block(self.id, block_handle.offset())
        {
            return Ok(block);
        }
        ...
        self.opts
            .block_cache
            .insert(self.id, block_handle.offset(), Item::Index(block.clone()));
...
mod tests {
    ...
    fn test_partitioned_index_lookup() {
        ...
        let (top_level_handle, _) = writer
            .finish(&mut buffer, CompressionType::None, 0)
            .unwrap();
        ...
        assert!(
            block.size() > 0,
            "Should find first block for key before range"
        );
