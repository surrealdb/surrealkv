...
impl LeafNode {
    ...
    fn delete(&mut self, key: &[u8], compare: &CompareFunction) -> Option<(usize, Vec<u8>)> {
        let idx = self
            .keys
            .iter()
            .position(|k| compare(key, k) == Ordering::Equal)?;
...
impl<'a, S: Storage> RangeScanIterator<'a, S> {
    pub fn new(tree: &'a mut BPlusTree<S>, start_key: &[u8], end_key: &[u8]) -> Result<Self> {
        ...
        let current_idx = leaf
            .keys
            .partition_point(|k| (tree.compare)(k, start_key) == Ordering::Less);
...
impl<'a, S: Storage> PrefixScanIterator<'a, S> {
    pub fn new(tree: &'a mut BPlusTree<S>, prefix: &[u8]) -> Result<Self> {
        ...
        let current_idx = leaf
            .keys
            .partition_point(|k| (tree.compare)(k, prefix) == Ordering::Less);
...
mod tests {
    ...
    fn test_delete_persistence() -> Result<()> {
        ...
        // Reopen and verify deletion persisted
        {
            ...
            assert_eq!(
                tree.search(b"two")?,
                None,
                "Deleted key still exists after reopening"
            );
            assert_eq!(
                tree.search(b"one")?,
                Some(b"1".to_vec()),
    ...
    fn test_range_basic() {
        ...
        // Insert test data
        for i in 1..=10 {
            tree.insert(&serialize_u32(i), &serialize_u32(i * 10))
                .unwrap();
        }
        ...
        assert_eq!(
            results
                .into_iter()
                .map(|res| res.map(deserialize_pair).unwrap())
                .collect::<Vec<_>>(),
            expected
        );
    }
    ...
    fn test_full_range() {
        ...
        let results: Vec<_> = tree
            .range(&serialize_u32(1), &serialize_u32(10))
            .unwrap()
            .collect();
    ...
    fn test_invalid_range() {
        ...
        let results: Vec<_> = tree
            .range(&serialize_u32(3), &serialize_u32(1))
            .unwrap()
            .collect();
    ...
    fn test_exact_match_range() {
        ...
        // Single key range
        let results: Vec<_> = tree
            .range(&serialize_u32(5), &serialize_u32(5))
            .unwrap()
            .collect();
        ...
        let results: Vec<_> = tree
            .range(&serialize_u32(3), &serialize_u32(3))
            .unwrap()
            .collect();
        assert!(results.is_empty());
    }
    ...
    fn test_prefix_scan_basic() {
        ...
        assert_eq!(
            iter.next().unwrap().unwrap(),
            (b"application".to_vec(), b"long".to_vec())
        );
        assert!(iter.next().is_none());
    }
    ...
    fn test_trunk_page_free_list_management() {
        ...
        // Create a new B+ tree
        {
            ...
            assert_eq!(
                tree.header.free_page_count, 0,
                "Should have no free pages initially"
            );
            assert_eq!(
                tree.header.trunk_page_head, 0,
                "Should have no trunk pages initially"
            );
            ...
            assert!(
                tree.header.free_page_count > 0,
                "Should have free pages after deletion"
            );
            assert!(
                tree.header.trunk_page_head > 0,
                "Should have at least one trunk page"
            );
        ...
        // Step 8: Reopen and verify trunk pages are preserved
        {
            ...
            assert!(
                tree.header.free_page_count > 0,
                "Free pages should be preserved after reopen"
            );

            // Verify trunk page chain
            assert!(
                tree.header.trunk_page_head > 0,
                "Trunk page head should be preserved"
            );
