...
impl MemTable {
    ...
    fn apply_batch_to_memtable(&self, batch: &Batch, starting_seq_num: u64) -> Result<(u32, u64)> {
        ...
        for record in batch.iter() {
            ...
            let val: Arc<[u8]> = Arc::from(val_slice.to_vec().into_boxed_slice());
    ...
    fn update_memtable_size(&self, record_size: u32) -> u32 {
        self.map_size
            .fetch_add(record_size, std::sync::atomic::Ordering::AcqRel)
    }
    ...
    pub fn add_entry(
        ...
        let val: Arc<[u8]> = Arc::from(value.unwrap_or(&[]).to_vec().into_boxed_slice());
...
mod tests {
    ...
    fn memtable_get() {
        ...
        assert_eq!(Arc::from(&value[..]), res.1);
    }
    ...
    fn memtable_add_and_get() {
        ...
        assert_eq!(Arc::from(&value1[..]), res.1);
        let res = memtable.get(b"key2", None).unwrap();
        assert_eq!(Arc::from(&value2[..]), res.1);
    }
    ...
    fn memtable_get_latest_seq_no() {
        ...
        let res = memtable.get(b"key1", None).unwrap();
        assert_eq!(Arc::from(value3), res.1);
    }
    ...
    fn test_sequence_number_ordering() {
        ...
        for (key, value) in &entries {
            ...
            if user_key.as_ref() == b"key1" {
                key1_entries.push((seq_num, value.clone()));
            }
        }
    ...
    fn test_key_updates_with_sequence_numbers() {
        ...
        assert!(result.is_some());
        let (_, val) = result.unwrap();
        assert_eq!(&*val, b"old_value"); // Should get the value with seq_num <= 8
    }
    ...
    fn test_tombstones() {
        ...
        assert_eq!(
            key_counts[&Arc::from(b"key1".to_vec().into_boxed_slice())],
            1
        );
        assert_eq!(
            key_counts[&Arc::from(b"key2".to_vec().into_boxed_slice())],
            2
        ); // Original + tombstone
        assert_eq!(
            key_counts[&Arc::from(b"key3".to_vec().into_boxed_slice())],
            1
        );
    }
    ...
    fn test_key_kinds() {
        ...
        assert_eq!(val.len(), 0); // Delete tombstone has empty value
    }
    ...
    fn test_range_query() {
        ...
        let user_keys: Vec<_> = range_entries
            .iter()
            .map(|(key, _)| key.user_key.clone())
            .collect();
    ...
    fn test_range_query_with_sequence_numbers() {
        ...
        for (key, value) in &range_entries {
            let (user_key, seq_num, _) = (key.user_key.clone(), key.seq_num(), key.kind());
            entries_info.push((user_key, seq_num, value.clone()));
        }
    ...
    fn test_range_bounds_with_internal_keys() {
        ...
        memtable
            .add_entry(InternalKeyKind::Set, b"a", Some(b"value_a1"), 5)
            .unwrap();
        memtable
            .add_entry(InternalKeyKind::Set, b"a", Some(b"value_a2"), 3)
            .unwrap();
        memtable
            .add_entry(InternalKeyKind::Set, b"b", Some(b"value_b1"), 7)
            .unwrap();
        memtable
            .add_entry(InternalKeyKind::Set, b"b", Some(b"value_b2"), 4)
            .unwrap();
        memtable
            .add_entry(InternalKeyKind::Set, b"c", Some(b"value_c1"), 6)
            .unwrap();
        memtable
            .add_entry(InternalKeyKind::Set, b"d", Some(b"value_d1"), 2)
            .unwrap();
        ...
        let keys: Vec<_> = range_inclusive
            .iter()
            .map(|(k, _)| (k.user_key.as_ref(), k.seq_num()))
            .collect();
        ...
        let keys: Vec<_> = range_exclusive
            .iter()
            .map(|(k, _)| (k.user_key.as_ref(), k.seq_num()))
            .collect();
        assert_eq!(keys[0], (b"a" as &[u8], 5));
        ...
        let keys: Vec<_> = range_start_excluded
            .iter()
            .map(|(k, _)| (k.user_key.as_ref(), k.seq_num()))
            .collect();
