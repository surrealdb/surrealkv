...
impl Options {
	...
	/// Sets the unified block cache capacity for data and index blocks.
...
pub trait LSMIterator {
	...
	/// Returns the raw inline bytes for the current value.
	fn value_encoded(&self) -> Result<&[u8]>;

	/// Get the current value as owned bytes.
	///
	/// This clones the raw inline bytes exposed by [`LSMIterator::value_encoded`].
	///
