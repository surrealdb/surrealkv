...
impl Writer {
	...
	/// Writes a record WITHOUT flushing the buffer, regardless of `manual_flush`.
	///
	/// Used by group commit: the commit thread appends every group member with
	/// this and then calls `write_buffer()` (and optionally `sync()`) ONCE for
	/// the whole group, coalescing the `write()` syscall. The caller MUST flush
	/// afterward.
