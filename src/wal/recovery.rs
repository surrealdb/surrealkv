...
mod tests {
	...
	use crate::wal::manager::Wal;
	use crate::wal::Options;
	use crate::{IntoBytes, WalRecoveryMode};
	...
	fn test_replay_wal_sequence_number_tracking() {
		...
		batch1.set(b"key1".into_bytes(), b"value1".into_bytes(), 0).unwrap(); // seq_num 100
		batch1.set(b"key2".into_bytes(), b"value2".into_bytes(), 0).unwrap(); // seq_num 101
		batch1.set(b"key3".into_bytes(), b"value3".into_bytes(), 0).unwrap(); // seq_num 102
																		// Highest sequence number should be 102

		// Batch 2: Starting at 200, with 4 entries (200, 201, 202, 203)
		let mut batch2 = Batch::new(200);
		batch2.set(b"key4".into_bytes(), b"value4".into_bytes(), 0).unwrap(); // seq_num 200
		batch2.set(b"key5".into_bytes(), b"value5".into_bytes(), 0).unwrap(); // seq_num 201
		batch2.delete(b"key6".into_bytes(), 0).unwrap(); // seq_num 202
		batch2.set(b"key7".into_bytes(), b"value7".into_bytes(), 0).unwrap(); // seq_num 203
	...
	fn test_replay_wal_single_entry_batches() {
		...
		batch1.set(b"key1".into_bytes(), b"value1".into_bytes(), 0).unwrap(); // seq_num 500

		let mut batch2 = Batch::new(600);
		batch2.set(b"key2".into_bytes(), b"value2".into_bytes(), 0).unwrap(); // seq_num 600

		let mut batch3 = Batch::new(700);
		batch3.set(b"key3".into_bytes(), b"value3".into_bytes(), 0).unwrap(); // seq_num 700
	...
	fn test_replay_wal_multiple_batches() {
		...
		batch1.set(b"key1".into_bytes(), b"value1".into_bytes(), 0).unwrap(); // seq_num 200
		batch1.set(b"key2".into_bytes(), b"value2".into_bytes(), 0).unwrap(); // seq_num 201
																		// Highest sequence number should be 201

		let mut batch2 = Batch::new(300); // Starting sequence number 300
		batch2.set(b"key3".into_bytes(), b"value3".into_bytes(), 0).unwrap(); // seq_num 300
		batch2.set(b"key4".into_bytes(), b"value4".into_bytes(), 0).unwrap(); // seq_num 301
	...
	fn test_post_corruption_replay() {
		...
		batch1.set(b"key2".into_bytes(), b"value2".into_bytes(), 0).unwrap();
	...
	fn test_repair_with_three_batches_corrupt_third() {
		...
		batch2.set(b"key4".into_bytes(), b"value4".into_bytes(), 0).unwrap();

		let mut batch3 = Batch::new(300);
		batch3.set(b"key5".into_bytes(), b"value5".into_bytes(), 0).unwrap();
		batch3.set(b"key6".into_bytes(), b"value6".into_bytes(), 0).unwrap();
	...
	fn test_recovery_mode_skip_corrupted() {
		...
		batch.set(b"key".into_bytes(), b"value".into_bytes(), 0).unwrap();
	...
	fn test_corruption_stops_at_segment_n() {
		...
		batch0.set(b"key0".into_bytes(), b"value0".into_bytes(), 0).unwrap();
