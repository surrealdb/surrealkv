...
pub struct Options {
	...
	#[cfg_attr(not(unix), allow(dead_code))] // read under #[cfg(unix)] in Wal::prepare_directory
	pub(crate) dir_mode: Option<u32>,

	/// The file mode to set for the segment file (Unix only).
	#[cfg_attr(not(unix), allow(dead_code))] // read under #[cfg(unix)] in Wal::open_wal_file
