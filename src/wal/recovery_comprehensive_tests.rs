...
use crate::wal::test_helpers::{CorruptionType, WalTestHelper};
...
fn test_sequential_5_segments_recovery() {
	...
	WalTestHelper::verify_entries(&memtable, &expected_keys);
}
...
fn test_single_large_vs_multiple_small_segments() {
	...
	WalTestHelper::verify_entry_count(&memtable1, total_entries);
	WalTestHelper::verify_entry_count(&memtable2, total_entries);
...
fn test_crash_immediately_after_rotation() {
	...
	for i in 0..50 {
		batch.set(format!("key{}", i).as_bytes(), format!("value{}", i).as_bytes(), 0).unwrap();
	}
...
fn test_crash_after_multiple_rapid_rotations() {
	...
	for i in 0..5 {
		...
		batch.set(format!("key{}", i).as_bytes(), format!("val{}", i).as_bytes(), 0).unwrap();
		wal.append(&batch.encode().unwrap()).unwrap();
...
fn test_crash_during_wal_write_mid_batch() {
	...
	for i in 0..10 {
		batch1.set(format!("key{}", i).as_bytes(), format!("val{}", i).as_bytes(), 0).unwrap();
	}
	...
	for i in 0..10 {
		batch2.set(format!("key2_{}", i).as_bytes(), format!("val2_{}", i).as_bytes(), 0).unwrap();
	}
...
fn test_very_large_values_across_segments() {
	...
	for seg_idx in 0..5 {
		...
		for i in 0..5 {
			...
			batch.set(key.as_bytes(), &large_value, 0).unwrap();
		}
		wal.append(&batch.encode().unwrap()).unwrap();
...
fn test_many_small_batches() {
	...
	for seg_idx in 0..10 {
		for batch_idx in 0..100 {
			...
			batch.set(key.as_bytes(), b"value", 0).unwrap();
			wal.append(&batch.encode().unwrap()).unwrap();
			entry_count += 1;
		}
...
fn test_compressed_wal_segments() {
	...
	opts.compression_type = Some(CompressionType::Lz4);
...
fn test_mixed_compression_segments() {
	...
	for seg_idx in 0..2 {
		...
		for i in 0..40 {
			...
			batch.set(key.as_bytes(), b"uncompressed_value", 0).unwrap();
		}
		wal.append(&batch.encode().unwrap()).unwrap();
		wal.rotate().unwrap();
	}
	...
	for seg_idx in 0..2 {
		...
		for i in 0..40 {
			...
			batch.set(key.as_bytes(), b"compressed_value", 0).unwrap();
		}
		wal.append(&batch.encode().unwrap()).unwrap();
		if seg_idx < 1 {
			wal.rotate().unwrap();
...
fn test_compressed_data_corruption() {
	...
	for i in 0..60 {
		...
		batch.set(key.as_bytes(), b"compressed_value_data", 0).unwrap();
	}
	wal.append(&batch.encode().unwrap()).unwrap();
	wal.close().unwrap();
