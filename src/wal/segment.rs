...
const DEFAULT_FILE_SIZE: u64 = 100 * 1024 * 1024; // 100mb
...
impl Metadata {
	...
	fn new_file_header(id: u64, opts: &Options) -> Result<Self> {
		...
		let cf = opts.compression_format.as_ref().unwrap_or(&CompressionFormat::NoCompression);
		let cl = opts.compression_level.as_ref().unwrap_or(&CompressionLevel::BestSpeed);
	...
	pub(crate) fn get_uint(&self, key: &str) -> Result<u64> {
		...
		let value_bytes = self
			.get(key)
			.ok_or(Error::IO(IOError::new(io::ErrorKind::NotFound, "Key not found")))?;

		let bytes: &[u8] = value_bytes.as_slice();
		let value_bytes = bytes.as_ref().try_into().map_err(|_| {
			Error::IO(IOError::new(io::ErrorKind::InvalidData, "Failed to convert bytes to u64"))
		})?;
...
fn validate_metadata(header: &[u8], opts: &Options) -> Result<()> {
	...
	match (additional_md, &opts.metadata) {
		(Some(md), Some(expected_md)) if *md != expected_md.to_bytes()? => {
			Err(Error::IO(IOError::new(io::ErrorKind::InvalidData, "Corrupted metadata")))
		}
		(None, Some(_)) | (Some(_), None) => {
			Err(Error::IO(IOError::new(io::ErrorKind::InvalidData, "Invalid metadata")))
		}
		_ => Ok(()),
	}
}
...
fn validate_magic_version(header: &[u8]) -> Result<()> {
	...
	if magic != MAGIC || version != VERSION {
		return Err(Error::IO(IOError::new(io::ErrorKind::InvalidData, "Invalid header data")));
	}
...
fn validate_segment_id(header: &[u8], id: u64) -> Result<()> {
	...
	if segment_id != id {
		return Err(Error::IO(IOError::new(io::ErrorKind::InvalidData, "Invalid segment ID")));
	}
...
impl<const RECORD_HEADER_SIZE: usize> Segment<RECORD_HEADER_SIZE> {
	pub(crate) fn open(dir: &Path, id: u64, opts: &Options) -> Result<Self> {
		...
		file.sync_all()?; // Ensure the file is synced before proceeding
	...
	fn flush(&mut self) -> Result<()> {
		if self.block.written > 0 {
			// Flush the full block to disk with zero padding to the end of the
			// last block. This is done to avoid writing partial records to the WAL,
			// and for detecting corruption.
...
impl MultiSegmentReader {
	...
	fn fill_with_zeros(&self, buf: &mut [u8], bytes_read: usize) -> usize {
		let mut i = 0;
...
mod tests {
	...
	fn read_reader_field() {
		let data = [0, 0, 0, 5, 65, 66, 67, 68, 69]; // "ABCDE" with big-endian length
	...
	fn metadata_extension() {
		...
		meta.read_from(&mut &extended_bytes[..]).expect("Failed to read from bytes");
	...
	fn check_and_validate_file_header() {
		...
		file.seek(io::SeekFrom::Start(0)).expect("should seek to start"); // Reset the cursor
	...
	fn test_append() {
		...
		// 7 + 4 + 7 + 7 = 25 (includes WAL headers)
		...
		// Validate offset after appending
		// BLOCK_SIZE + 7 + 4 = 32768 + 11
		assert_eq!(segment.offset(), BLOCK_SIZE as u64 + 7 + 4);

		// Test reading from segment after appending
		let mut bs = vec![0; 11];
		let n = segment.read_at(&mut bs, BLOCK_SIZE as u64).expect("should read");
		assert_eq!(11, n);
	...
	fn segment_append_read_append() {
		...
		// Read data from a specific position (skip WAL header)
		let mut buffer = vec![0u8; 2];
		let read_result = segment.read_at(&mut buffer, WAL_RECORD_HEADER_SIZE as u64);
		...
		// Read data from a specific position again (skip second WAL header)
		let mut buffer = vec![0u8; 3];
		let read_result = segment
			.read_at(&mut buffer, (WAL_RECORD_HEADER_SIZE + 4 + WAL_RECORD_HEADER_SIZE) as u64);
		assert!(read_result.is_ok());
	...
	fn multi_segment() {
		...
		let segments: Vec<SegmentRef> =
			vec![create_test_segment_ref(&segment1), create_test_segment_ref(&segment2)];
	...
	fn partial_block() {
		...
		buf_reader.read(&mut read_buffer).expect_err("should not read");
	...
	fn sync_on_synced_segment() {
		...
		let sz = segment.offset();
		assert_eq!(BLOCK_SIZE as u64, sz);
	...
	fn multi_segment_rec() {
		...
		// Read first block (contains WAL header + [0,1,2,3] + padding)
		let mut block1 = [0u8; BLOCK_SIZE];
		let bytes_read = buf_reader.read(&mut block1).expect("should read");
		assert_eq!(bytes_read, BLOCK_SIZE);
	...
	fn file_option_with_append_mode() {
		...
		// Open the file with append and read permissions
		let mut file = OpenOptions::new().read(true).append(true).open(&file_path).unwrap();
	...
	fn append_mode_seek_and_write() {
		...
		let mut file = OpenOptions::new().create(true).read(true).append(true).open(&path).unwrap();
