...
mod tests {
    ...
    fn segment_append_read_append() {
        ...
        let read_result = segment.read_at(
            &mut buffer,
            (WAL_RECORD_HEADER_SIZE + 4 + WAL_RECORD_HEADER_SIZE) as u64,
        );
        assert!(read_result.is_ok());
    ...
    fn multi_segment_rec() {
        ...
        assert_eq!(
            &[0, 1, 2, 3],
            &block1[WAL_RECORD_HEADER_SIZE..WAL_RECORD_HEADER_SIZE + 4]
        );
        ...
        assert_eq!(
            &[4, 5, 6, 7],
            &block2[WAL_RECORD_HEADER_SIZE..WAL_RECORD_HEADER_SIZE + 4]
        );
        ...
        assert_eq!(
            &[8, 9],
            &block2[second_record_start..second_record_start + 2]
        );
    }
