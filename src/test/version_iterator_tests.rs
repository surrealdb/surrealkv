...
//!
//! These exercise LSM-based versioned iteration over memtables and SSTables.
...
async fn test_history_soft_delete_vs_hard_delete() {
...

	{
		...
		assert_eq!(results.len(), 2, "Only key1 visible");
		assert_eq!(results[0].0, b"key1");
...
async fn test_history_many_versions() {
	println!("test_history_many_versions ");
	let (store, _temp_dir) = create_versioned_store();
...
async fn test_history_timestamps() {
...

	{
		...
		// Verify ordering is by timestamp descending
...
async fn test_get_at_ryow() {
	...
	assert_eq!(value, Some(b"uncommitted_v2".to_vec()), "");

	let value = tx.get_at(b"key2", 150).unwrap();
	assert_eq!(value, Some(b"uncommitted_new".to_vec()), "");

	let value = tx.get_at(b"key1", 100).unwrap();
	assert_eq!(value, Some(b"committed_v1".to_vec()), "");
}
...
async fn test_get_at_ryow_future_timestamp() {
	...
	assert_eq!(value, None, "Should not see future write");
...
async fn test_history_ryow_timestamp_collision() {
...

	{
		...
		assert_eq!(results.len(), 1, "Uncommitted shadows committed");
		assert_eq!(results[0].0, b"key1".to_vec());
...
async fn test_history_ryow_with_ts_range() {
...

	{
		...
		assert_eq!(results.len(), 3, "Should have 3 entries in range");
		assert_eq!(results[0].2, 200);
...
async fn test_history_ryow_soft_delete() {
...

	{
		...
		assert_eq!(results.len(), 1, "Soft delete filtered");
		...
		assert_eq!(results.len(), 2, "Should have 2 with tombstones");
		assert_eq!(results[0].2, 200);
...
async fn test_history_ryow_hard_delete() {
...

	{
		...
		assert_eq!(results.len(), 1, "key1 wiped by hard delete");
		...
		assert_eq!(results.len(), 1, "Hard delete wipes even with tombstones");
		assert_eq!(results[0].0, b"key2".to_vec());
	}
...
async fn test_history_tombstone_at_lower_bound() {
	...
	assert!(
		...
		"Without tombstones flag, older value version should still appear"
	);
	// The tombstone at ts=2 should NOT appear
	assert!(
		!results.iter().any(|(k, _, ts, is_tomb)| k == b"key_a" && *ts == 2 && *is_tomb),
		"Without tombstones flag, tombstone should be filtered out"
	);
	...
	assert!(
		results.iter().any(|(k, _, ts, is_tomb)| k == b"key_a" && *ts == 2 && *is_tomb),
		"With tombstones flag, tombstone at lower bound should appear"
	);
