...
use crate::{CompressionType, IntoBytes, Options};
...
fn build_table_with_compression(
...

	{
		...
		for (k, v) in data {
			builder.add(InternalKey::new(k.into_bytes(), 1, InternalKeyKind::Set, 0), &v).unwrap();
		}
...
fn test_compression_10k_pairs_roundtrip() {
	...
	for _ in 0..100 {
		...
		let seek_key =
			InternalKey::new(data[idx].0.clone().into_bytes(), 2, InternalKeyKind::Set, 0);
...
fn test_compression_iterator_operations() {
	...
	let mid_key = InternalKey::new(data[5000].0.clone().into_bytes(), 2, InternalKeyKind::Set, 0);
...
fn test_table_writer_with_level_compression() {
...

	{
		...
		for (key, value) in data {
			let ikey = InternalKey::new(key.clone().into_bytes(), 1, InternalKeyKind::Set, 0);
