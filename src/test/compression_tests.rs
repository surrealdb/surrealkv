...
fn test_compression_10k_pairs_roundtrip() {
	...
	for _ in 0..100 {
		...
		assert_eq!(iter.raw_value().unwrap(), &data[idx].1[..]);
	}
}
...
fn test_compression_size_reduction() {
	...
	while iter_uncompressed.valid() && iter_compressed.valid() {
		...
		assert_eq!(iter_uncompressed.raw_value().unwrap(), iter_compressed.raw_value().unwrap());
		count += 1;
...
fn test_compression_mixed_patterns() {
	...
	while iter.valid() {
		...
		assert_eq!(iter.raw_value().unwrap(), &data[count].1[..]);
		count += 1;
		iter.next().unwrap();
	}
...
fn test_compression_iterator_operations() {
	...
	assert_eq!(iter.raw_value().unwrap(), &data[0].1[..]);

	iter.seek_to_last().unwrap();
	assert!(iter.valid());
	assert_eq!(iter.key().user_key(), &data[9999].0[..]);
	assert_eq!(iter.raw_value().unwrap(), &data[9999].1[..]);
...
fn test_table_writer_with_level_compression() {
	...
	assert_eq!(iter.raw_value().unwrap(), b"value1");
}
