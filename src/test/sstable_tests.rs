...
fn test_table_seek() {
	...
	assert!(iter.valid());
	assert_eq!((iter.key().user_key(), iter.raw_value().unwrap()), (&b"bcd"[..], &b"asa"[..]));

	let key = InternalKey::new(Vec::from(b"abc"), 2, InternalKeyKind::Set, 0);
	iter.seek(&key.encode()).unwrap();
	assert!(iter.valid());
	assert_eq!((iter.key().user_key(), iter.raw_value().unwrap()), (&b"abc"[..], &b"def"[..]));
...
fn test_table_iter() {
	...
	iter.next().unwrap();
	assert!(iter.valid());
	assert_eq!((iter.key().user_key(), iter.raw_value().unwrap()), (&b"abd"[..], &b"dee"[..]));
	...
	iter.next().unwrap();
	assert!(iter.valid());
	assert_eq!((iter.key().user_key(), iter.raw_value().unwrap()), (&b"bsr"[..], &b"a00"[..]));

	iter.next().unwrap();
	assert!(iter.valid());
	assert_eq!((iter.key().user_key(), iter.raw_value().unwrap()), (&b"xyz"[..], &b"xxx"[..]));

	iter.next().unwrap();
	assert!(iter.valid());
	assert_eq!((iter.key().user_key(), iter.raw_value().unwrap()), (&b"xzz"[..], &b"yyy"[..]));

	iter.next().unwrap();
	assert!(iter.valid());
	assert_eq!((iter.key().user_key(), iter.raw_value().unwrap()), (&b"zzz"[..], &b"111"[..]));
}
...
fn test_iter_items() {
	...
	while iter.valid() {
		...
		let value = iter.raw_value().unwrap();
...
fn test_table_iterator_seek_then_iterate() {
	...
	for (seek_key, expected_start_index) in test_cases {
		...
		while iter.valid() {
			let current_key = iter.key();
			let current_value = iter.raw_value().unwrap();
			let key_str = std::str::from_utf8(current_key.user_key()).unwrap();
...
fn test_table_iterator_next_vs_advance_consistency() {
	...
	while iter1.valid() {
		collected_via_advance.push((iter1.key().to_owned(), iter1.raw_value().unwrap().to_vec()));
	...
	while iter2.valid() {
		collected_via_next.push((iter2.key().to_owned(), iter2.raw_value().unwrap().to_vec()));
...
fn test_table_iter_both_bounds_excluded_same_key_reverse() {
	...
	iter.seek_last().unwrap();
	while iter.valid() {
		results.push((iter.key().to_owned(), iter.raw_value().unwrap().to_vec()));
		if !iter.prev().unwrap() {
			break;
		}
	}
...
fn test_single_entry_table() {
	...
	let iter_value = iter.raw_value().unwrap();
