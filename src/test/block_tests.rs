...
fn test_block_iter() {
	...
	while block_iter.advance().unwrap() {
		...
		assert_eq!(block_iter.value().unwrap(), data[i].1.to_vec());
		i += 1;
	}
...
fn test_block_iter_reverse() {
	...
	assert_eq!(iter.value().unwrap(), b"value1".to_vec());

	// Advance to second entry (loooongkey1)
	iter.next().unwrap();
	assert!(iter.valid());
	assert_eq!(iter.key().user_key(), "loooongkey1".as_bytes());
	assert_eq!(iter.value().unwrap(), b"value2".to_vec());
	...
	assert_eq!(iter.value().unwrap(), b"value".to_vec());
...
fn test_block_seek() {
	...
	assert_eq!(
		Some((block_iter.key().user_key(), block_iter.value().unwrap(),)),
...
fn test_block_seek_to_last() {
	...
	for block_restart_interval in [2, 6, 10] {
		...
		assert_eq!(block_iter.value().unwrap(), b"value".to_vec());

		block_iter.seek_to_first().unwrap();
		assert!(block_iter.valid());
		assert_eq!(block_iter.key().user_key(), "key1".as_bytes());
		assert_eq!(block_iter.value().unwrap(), b"value1".to_vec());
