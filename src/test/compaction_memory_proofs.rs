...
/// B-P1: the partitioned filter builder buffers an 8-byte HASH per key
...
/// residency — the root memory bug of issue #397.
#[test]
fn proof_bp1_filter_buffers_every_key_until_finish() {
	...
	// Post-fix bounds: the filter retains ~6-8 B/key (hash + finished bloom bits + buffer
	// overheads) instead of 46+ B/key of raw keys.
	assert!(
		filter_share <= 400_000 * 12,
		"filter memory exceeds 12 B/key (share = {filter_share} B at 400k keys) — the raw-key \
		 buffering bug (issue #397) is back"
	);
	assert!(
		small_4n <= 400_000 * 16,
		"small regime: writer retains {small_4n} B at 400k entries, expected <= 16 B/key"
	);
	assert!(
		large_4n <= 32_000 * 24,
		"large regime: writer retains {large_4n} B at 32k entries, expected <= 24 B/key"
	);
...
fn proof_gb_scale_legacy_filter_needs_1gb_partitioned_does_not() {
	...
	eprintln!(
		"GB-scale: legacy peak = {old_peak} B ({:.2} GB), partitioned peak = {new_peak} B ({:.2} MB)",
		old_peak as f64 / (1024.0 * 1024.0 * 1024.0),
		new_peak as f64 / (1024.0 * 1024.0)
	);

	assert!(new_peak < old_peak / 10, "partitioned writer should use < 10% of legacy memory");
}
