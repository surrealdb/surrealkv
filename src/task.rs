...
impl fmt::Debug for TaskManager {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("TaskManager")
            .field(
                "memtable_running",
                &self.memtable_running.load(Ordering::Acquire),
            )
...
mod tests {
    ...
    async fn test_concurrent_wake_up_memtable() {
        ...
        Arc::try_unwrap(task_manager)
            .expect("Task manager still has references")
            .stop()
            .await;
    }
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn test_chained_operation_ordering() {
        // Create a mock with controllable delays to test ordering
        let core = Arc::new(MockCoreInner::with_delays(50, 50));

        // Create counters to track operation order
        let memtable_done_at = Arc::new(AtomicUsize::new(0));
        let level_done_at = Arc::new(AtomicUsize::new(0));
        let operation_counter = Arc::new(AtomicUsize::new(0));

        // Create a custom core that tracks operation order
        struct OrderTrackingCore {
            inner: Arc<MockCoreInner>,
            memtable_done_at: Arc<AtomicUsize>,
            level_done_at: Arc<AtomicUsize>,
            operation_counter: Arc<AtomicUsize>,
        }

        impl CompactionOperations for OrderTrackingCore {
            fn compact_memtable(&self) -> Result<()> {
                let result = self.inner.compact_memtable();
                // Record when this operation completed
                let counter = self.operation_counter.fetch_add(1, Ordering::SeqCst);
                self.memtable_done_at.store(counter, Ordering::SeqCst);
                result
            }

            fn compact(&self, strategy: Arc<dyn CompactionStrategy>) -> Result<()> {
                let result = self.inner.compact(strategy);
                // Record when this operation completed
                let counter = self.operation_counter.fetch_add(1, Ordering::SeqCst);
                self.level_done_at.store(counter, Ordering::SeqCst);
                result
            }
        }

        let tracking_core = Arc::new(OrderTrackingCore {
            inner: core.clone(),
            memtable_done_at: memtable_done_at.clone(),
            level_done_at: level_done_at.clone(),
            operation_counter: operation_counter.clone(),
        });

        let task_manager = TaskManager::new(tracking_core);

        // Trigger memtable compaction which should chain to level
        task_manager.wake_up_memtable();
        time::sleep(Duration::from_millis(200)).await; // Wait for both operations

        // Check that memtable compaction completed before level compaction
        assert!(
            memtable_done_at.load(Ordering::SeqCst) < level_done_at.load(Ordering::SeqCst),
            "Memtable compaction should complete before level compaction"
        );
