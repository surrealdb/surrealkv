...
impl Transaction {
        ...
        R: RangeBounds<&'b [u8]>,
    {
        ...
        // Do not allow scans if it is a write-only transaction
        if self.mode.is_write_only() {
            return Err(Error::TransactionWriteOnly);
        }
    ...
    fn is_key_in_range<'b, R>(key: &[u8], range: &R) -> bool
    where
        R: RangeBounds<&'b [u8]>,
    {
