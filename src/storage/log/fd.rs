...
use crate::storage::log::{Error, Options, Result, Segment};
...
impl FileHandle {
    ...
    fn get_fd(&mut self) -> Result<Arc<SysFile>> {
        if let Some(fd) = self
            .file_descriptors
            .iter()
            .find(|sf| !sf.in_use.load(Ordering::SeqCst))
        {
            fd.in_use.store(true, Ordering::SeqCst);
            return Ok(fd.clone());
        }
        if self.file_descriptors.len() < self.max_fds {
            let new_file = SysFile::new(&self.dir, self.segment_id, self.opts.clone())?.into();
...
mod tests {
    ...
    fn create_segment_file(temp_dir: &TempDir, id: u64) -> Arc<Options> {
        ...
        let mut segment =
            Segment::open(temp_dir.path(), id, opts.clone()).expect("should create segment");
