...
pub(super) trait Routine {
    fn setup(&self, ds: Arc<Datastore>, num_ops: usize);
    fn run(&self, ds: Arc<Datastore>, num_ops: usize);
    fn cleanup(&self, ds: Arc<Datastore>, num_ops: usize);
}
    ...
    R: Routine,
{
    ...
    b.iter_custom(|iters| {
        ...
        for _ in 0..iters {
            // Setup
            routine.setup(ds.clone(), num_ops);

            // Run and time the routine
            let now = std::time::Instant::now();
            routine.run(ds.clone(), num_ops);
            total += now.elapsed();

            // Cleanup the database
            routine.cleanup(ds.clone(), num_ops);
        }
